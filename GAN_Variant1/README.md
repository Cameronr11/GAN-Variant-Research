...
The main configuration file is `configs/train_gan_cutpp.yaml`. Key sections:
...
model:
  generator:
    ...
    use_attention: false      # Baseline: disabled
    use_channel_attn: false   # Baseline: disabled
    use_style_dropout: false  # Baseline: disabled
  
  discriminator:
    base: "patchgan"
    num_scales: 1             # Baseline: single-scale
    ndf: 64
    n_layers: 3
    norm: "none"
    use_spectral_norm: false  # Baseline: disabled
    receptive_field: 70
```
...
```yaml
loss_weights:
  adv: 1.0              # Adversarial loss
  patchnce: 1.0         # PatchNCE contrastive loss
  identity_warm: 0.1    # Identity loss during warmup
  identity_final: 0.0   # Identity loss after warmup (disabled)
```
...
### PatchNCE Configuration

```yaml
patchnce:
  num_patches: 256
  temperature: 0.07
  nce_layers: [0, 4, 8, 12, 16]  # Generator layers to extract features from
...
```yaml
r1:
  gamma: 10.0    # R1 penalty weight
  every: 16      # Apply every N steps (lazy R1)

ema:
  decay: 0.999
  warmup_steps: 100

grad_clip_g: 10.0  # Generator gradient clipping
grad_clip_d: 10.0  # Discriminator gradient clipping
```
...
The generator's `get_feature_layers()` method extracts features at specific layers:

1. **Layer 0**: After initial 7×7 conv
2. **Layers 1-2**: After each downsampling layer
3. **Layers 3-11**: After each of the 9 residual blocks
4. **Layers 12-13**: After each upsampling layer

The PatchNCE loss uses layers `[0, 4, 8, 12, 16]` by default, extracting features at multiple scales to ensure both low-level and high-level correspondences.

### PatchNCE Sampling Strategy

- Randomly samples 256 patches per image (or all patches if image is small)
- Uses the same patch indices for both source and target features
- Normalizes features with L2 norm (epsilon=1e-6 for stability)
- Computes similarity matrix: `(num_patches, C) × (C, num_patches)`
- Applies temperature scaling: `logits / temperature`
- Clamps logits to [-50, 50] to prevent overflow in softmax

### EMA Implementation

The EMA (Exponential Moving Average) maintains a shadow copy of generator parameters:

```python
shadow_param = (1 - decay) * current_param + decay * shadow_param
```

- Updated every step after generator optimization
- Used for inference (more stable than raw generator)
- Decay: 0.999 (very slow update)
- Warmup: 100 steps before starting EMA updates

### Mixed Precision Training

- Uses PyTorch's native `torch.amp.autocast()` and `GradScaler`
- Automatic loss scaling prevents underflow
- Gradient unscaling before clipping (required for gradient clipping with AMP)
- FP32 used for identity loss (stability)

### Checkpoint Format

Checkpoints contain:
- `step`: Current training step
- `generator`: Generator state dict
- `discriminator`: Discriminator state dict
- `opt_G`: Generator optimizer state
- `opt_D`: Discriminator optimizer state
- `ema_G`: EMA state (decay, shadow parameters)
- `scaler`: AMP scaler state
- `config`: Full training configuration (for reproducibility)
- `metrics`: Optional metrics dictionary

### Loss Tracking

- Logs to CSV: `losses_history.csv` (step, d_loss, g_loss)
- Logs to text: `train_log.txt` (full loss breakdown)
- Generates plot: `losses_plot.png` (training curves)
...
**Random Seeds:**
- Set via `seed_dist.set_seed()` (default: 42)
- Ensures reproducible data loading, model initialization, and training

**Deterministic Operations:**
- PyTorch operations are deterministic where possible
- Some operations (e.g., cuDNN) may have non-deterministic behavior on GPU

**Checkpointing:**
- Full configuration saved in checkpoints
- Allows exact reproduction of training conditions
...
**Training Speed:**
- ~0.5-1.0 seconds per step (depends on GPU)
- Batch size: 12 (configurable)
- Image size: 256×256

**Memory Usage:**
- Generator: ~11M parameters
- Discriminator: ~2.7M parameters (single-scale)
- Total GPU memory: ~4-6 GB (with AMP, batch_size=12)

**Convergence:**
- Baseline typically reaches stable training after ~20k steps
- Full training: 70 epochs (configurable)
- Expected MiFID: 103-105 (as noted in config comments)
...
1. **Baseline CUT Implementation**: Clean, stable implementation of CUT with PatchNCE
2. **Training Stability**: Multiple techniques (EMA, gradient clipping, R1, NaN detection)
3. **Efficient Training**: Mixed precision, lazy R1, optimized data loading
4. **Modular Design**: Clean separation of concerns (models, losses, training, utils)

### Architecture Highlights

- **Generator**: ResNet-9 with 9 residual blocks, 2 downsampling/upsampling layers
- **Discriminator**: Single-scale PatchGAN (70×70 receptive field)
- **Feature Extraction**: Multi-layer feature extraction for PatchNCE (5 layers)

### Loss Function Highlights

- **PatchNCE**: Core contrastive loss enabling unpaired translation
- **Hinge Loss**: Stable adversarial training
- **Identity Loss**: Warmup stabilization
- **R1 Regularization**: Discriminator smoothness

### Training Highlights

- **DiffAugment**: Differentiable data augmentation
- **EMA**: Stable inference
- **Mixed Precision**: Efficient training
- **Lazy R1**: Cost-effective regularization
...
- Configuration file: `configs/train_gan_cutpp.yaml`
- Training script: `training/train_cutpp.py`
- Model definitions: `models/`

---

**Last Updated**: Based on implementation as of the current codebase state.
